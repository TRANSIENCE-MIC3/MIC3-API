...
all `KEYCLOAK_*` entries from `.env.example`. Replace the three example
Keycloak passwords with distinct local-only values. Never commit `.env`.
...
- the public `mic3-local` browser client using authorization code flow with
  PKCE and the `mic3-api` access-token audience;
...
only the `mic3` realm through the Admin Console and restart Keycloak:
